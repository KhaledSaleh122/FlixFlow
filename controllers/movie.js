...
import { getMovieGenreName } from './tools.js';
dotenv.config();
...
export async function getMovieById(id){
    try {
        const endpoint = `movie/${id}?${getMovieQueryUrl().toString()}`
...
export async function viewSimilar(req,res){
    ...
    const endpoint = `movie/${id}/recommendations?${url.toString()}`
    try {
...
export async function viewNewest(req,res){
    const {page} = req.query;
    const url = new URLSearchParams();
    url.append("api_key",APIKEY);
    url.append("page", page || 1);
    url.append("include_adult","false");
    url.append("include_video","true");
    const endpoint = `movie/now_playing?${url.toString()}`
    try {
        const response = await axios.get(tmdbURL+endpoint);
        addGenreNameAlongWithId(response);
        res.status(200).json(
...
export async function viewTrending(req,res){
    ...
    url.append("include_adult","false");
    url.append("include_video","true");
    const endpoint = `trending/movie/day?${url.toString()}`
    try {
        const response = await axios.get(tmdbURL+endpoint);
        addGenreNameAlongWithId(response);
        res.status(200).json(
...
export async function viewTopRated(req,res){
    ...
    url.append("include_adult","false");
    url.append("include_video","true");
    const endpoint = `movie/top_rated?${url.toString()}`
    try {
        const response = await axios.get(tmdbURL+endpoint);
        addGenreNameAlongWithId(response);
        res.status(200).json(
...
export async function viewAll(req,res){
    ...
    const endpoint = `discover/movie?${url.toString()}`
    try {
        const response = await axios.get(tmdbURL+endpoint);
        addGenreNameAlongWithId(response);
        res.status(200).json(
